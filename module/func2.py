...
# 看userid有沒有輸入過收入資料
def is_in_or_not_income(userid, types):
    url3 = 'https://docs.google.com/spreadsheets/d/1OGn7xzKwI8xySKstNWhpnqglK3AzooVPT11MCBAOGH4/export?format=csv'  # 下載連結
    webpage = urllib.request.urlopen(url3)  # 開啟網頁
    data3 = csv.DictReader(webpage.read().decode('utf-8-sig').splitlines())  # 讀取資料到data陣列中
    banana = [[i["userid"], i["金額"], i["時間"]] for i in data3]
    ...
    i = 0
    while i < len(banana):
        if userid in banana[i] and banana[i][2][:4]+banana[i][2][5:7] == types:
            reply = "good"
        i = i + 1
    return reply
...
# 找資料在收入試算表裡的第幾行
def find_row_income(userid, income, time):
    url2 = 'https://docs.google.com/spreadsheets/d/1OGn7xzKwI8xySKstNWhpnqglK3AzooVPT11MCBAOGH4/export?format=csv'  # 下載連結
    webpage = urllib.request.urlopen(url2)  # 開啟網頁
    data2 = csv.DictReader(webpage.read().decode('utf-8-sig').splitlines())  # 讀取資料到data陣列中
    banana = [[i["userid"], i["金額"], i["時間"]] for i in data2]
    i = 0
    b = []
    reply = -2
    while i < len(banana):
        if userid == banana[i][0] and income == banana[i][1] and time == banana[i][2][:4] + banana[i][2][5:7]:
            b.append(str(i))
            if len(b) > 0:
                reply = b[0]
