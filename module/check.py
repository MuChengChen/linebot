...
def check_number_records_in_spend_data(userid, text):
    ...
    if int(number) >= int(order):
        ...
        reply = '第{}筆資料為:'.format(number)
        reply = reply + '\n{}.{}.{} {}:{} {} {}支出 {}元'.format(data[int(number)][0], data[int(number)][1], data[int(number)][2], data[int(number)][3], data[int(number)][4], data[int(number)][5], data[int(number)][6], data[int(number)][7])
    return reply
...
def check_last_item_from_spend_data(userid, number, item):
    ...
    if data == {}:
        ...
        for i in range(order):
            reply = reply + '\n' + a + '\n{}.{}.{} {}:{} {}支出 {}元'.format(data[i][0], data[i][1], data[i][2], data[i][3], data[i][4], data[i][5], data[i][6])
    else:
